  ...
  {:else}
    ...
    {#if info.banner && !bannerError}
      <div class="relative h-48 md:h-64 overflow-hidden">
        <img src={info.banner} alt="" class="w-full h-full object-cover" onerror={() => bannerError = true} />
      ...
      <div class="flex flex-col md:flex-row gap-6 md:gap-8">
        ...
        <div class="w-40 md:w-64 flex-shrink-0 mx-auto md:mx-0">
          <div class="aspect-[3/4] rounded-xl overflow-hidden bg-zinc-800 shadow-xl shadow-black/40 border border-zinc-800/60">
            {#if cover && !coverError}
              <img src={cover} alt={title} class="w-full h-full object-cover" onerror={() => coverError = true} />
            {:else}
            ...
            {#each episodes as ep}
              ...
              >
                ...
                <div class="w-28 h-16 rounded-lg overflow-hidden bg-zinc-800 flex-shrink-0 relative">
                  {#if ep.thumbnail}
                    <img src={ep.thumbnail} alt="" class="w-full h-full object-cover" loading="lazy" onerror={(e) => e.target.style.display = 'none'} />
                  {/if}
                  {#if ep.duration}
